...
public class MemberDto {
	...
	// * 주로 DTO는 멤버변수 private 접근제한자 사용한다. private란? 다른 클래스에서 접근 불가능
		// -getter/setter 메소드 : private 멤버변수를 다른 클래스에서 간접접근하기 위한 메소드
		// -toString() 메소드 : 객체 호출시 참조(주소)값 대신 멤버변수 값을 반환하는 메소드 * 객체 내용을 확인용
	// * 주로 DTO 는 생성자를 기본적으로 2개( 디폴트 , 풀생성자 ) , 그외 커스텀 , 생성자 이란 ? new(인스턴스/객체)를 생성할때 사용되는 초기화 함수 역할 
		// - 오버로딩 : 동일한 메소드명을 여러개 선언가능, 전제조건 : 매개변수의 타입, 개수,순서 
	
	//1. 멤버변수/필드/속성 : 객체가 가질수 있는 고유한 값을 저장하는 속성명
	private int mno;
	private String mid;
	private String mpwd;
	...
	//3. 메소드 ,getter/setter , toString //자동생성 가능
	public int getMno() {
		return mno;
	}
