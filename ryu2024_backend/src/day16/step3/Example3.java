...
class Child extends Parent{ //하위 클래스
	...
	@Override //오버라이딩 어노테이션 //생략가능하다.
	//멤버변수가 동일하면 this/super, 메소드명이 동일하면 왜? this/super안쓰는 이유
	//멤버변수는 인스턴스마다 할당 , 메소드는 모든 인스턴스가 하나의 메소드를 공유해서 사용하기 때문
...
//		parent.method1();	   //Child method1, 타입이 변환된다고 해서 인스턴스 바뀌는건 아니다.
//		parent.method2();
//		//parent.method3(); //타입변환시 하위 타입의 멤버에 접근하라 수 없다.
//		
//		
//		//3. 부모 클래스의 타입에서 자식 클래스의 타입 변환
//		Child child2 = (Child)parent;	//부모타입 --> 자식타입OK //태생이 child라서
...
//			Child child3 = (Child)parent2;//부모타입 --> 자식타입 Fail //태생이 child가 아니라서
//			
//		}
//		else {
