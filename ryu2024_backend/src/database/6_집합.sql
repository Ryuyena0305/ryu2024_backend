# 1. 데이터베이스 준비
drop database if exists example6_집합; # 만약 db존재하면 삭제
create database example6_집합; #데이터베이스 생성
use example6_집합;	# 데이터베이스 사용 활성화
...
select * from table1;
select * from table2;
	# PK(기본키/식별키) 필드/속성 : 고유한 값들을 가진 필드 , 중복없음 unique, not null, ex) 주민등록번호, 사번, 학번 등
    # FK(왜래키/참조키) 필드/속성 : 다른 테이블의 속성값 참조하는 필드, 중복허용, null허용, 	 ex) 작성자, 주문한 제품번호
    
# 5. 집합 : 교집합 : 두 테이블간의 공통으로 포함하는 속성값으로 이루어진 테이블 조회
# 조회시 조회단위 : 레코드 단위
select * from table1;				# 레코드 5개 조회
select * from table1,table2;		# 2개 이상의 테이블 조회 : select * from 테이블명A,테이블명B

# 교집합
# INNER JOIN
# [1] where 이용한 교집합 표현하기, 두 테이블의 공통 속성값 num_pk와 num_fk, 주로 PK속성- FK속성, 테이블 겹치기
select * from table1, table2 where table1.num_pk = table2.num_pk;		#where 사용한 집합은 일반 조건과 겹치므로 구분이 어렵다.
# [2] inner join ~ on 절 이용한 교집합 표현하기 , 가독성, where 일반조건, on 집합조건, having : 그룹조건
# select * from 테이블명A inner join 테이블명B on 집합조건(테이블A.PK속성명 = 테이블.FK속성명);
...
# [4] using : 전제조건은 pk속성명과 fk속성명이 같아야한다. 현재 예제는 num_pk/num_fk 다르므로
select * from table1 join table2 using(num_pk);

# [5] natural join 키워드 : 두 테이블간의 동일한 소고성명 기준으로 자동으로 조인한다.
# 조인 의도와 다르게 동일한 속성명이 존재할 수 있으므로 비권장
select * from table1 natural join table2;

# OUTER JOIN
# [6] 왼쪽테이블(table1)에 있는 모든 레코드를 포함하고 오른쪽 테이블(table2)에 교집합되는 레코드 조회, 없으면 null
select * from table1 left outer join table2 on table1.num_pk = table2.num_pk;
# [7] 오른쪽테이블(table2)에 있는 모든 레코드를 포함하고 왼쪽 테이블(table1)에 교집합되는 레코드 조회, 없으면 null
...
# * 속성명 is null : 속성값이 null인지 아닌지 비교 연산, 속성명 is not null : 속성값이 null인지 비교연산
...
# [문제1] : 모든 제품(product)테이블 레코드 조회시 '카테고리번호' 대신에 '카테고리명'을 조회하는 SQL을 작성하시오.
select * from product,pcategory;			#카테고리명은 카테고리 테잉블에 존재한다. 두 테이블 조회를 해야한다.alter
# 두 테이블간의 관계 파악하여 교집합을 한다.
select * from product inner join pcategory on product.카테고리번호_fk = pcategory.카테고리번호_pk;	#두 테이블간의 관계 파악하여 교집합을 한다.
# 대신에 내가 원하는 속성명을 작성한다.
select product.제품번호_pk,product.제품명,product.제품가격,pcategory.카테고리명 from product inner join pcategory on product.카테고리번호_fk = pcategory.카테고리번호_pk;
# 별칭 : 속성명 별칭명 , 테이블명 별칭명
...
# [문제2] : 제품번호 1번의 재고수량 합계를 조회하는 SQL작성하시오. 그리고 조회시'제품명'과 '합계수량' 속성명으로 조회하시오
select * from stock;
